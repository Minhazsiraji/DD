        ...
        <div className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8 lg:p-10">
          <div className="flex min-w-0 flex-col items-center gap-5 text-center sm:flex-row sm:items-center sm:gap-7 sm:text-left">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex min-w-0 flex-wrap justify-center gap-x-3 gap-y-1 text-sm text-slate-600 sm:justify-start sm:text-base">
            ...
            >
              <div className="grid min-w-0 gap-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-stretch">
                <div className="min-w-0">
                  ...
                  {(chamber.address || chamber.district) && (
                    <p className="mt-1 break-words text-sm text-slate-600">
                      {[chamber.address, chamber.district].filter(Boolean).join(", ")}
                    </p>
                  )}
                  {chamber.publicNote && (
                    <p className="mt-2 break-words text-sm text-slate-500">{chamber.publicNote}</p>
                  )}
                    ...
                    <div className="mt-5 grid min-w-0 gap-2 sm:grid-cols-2">
                      {chamber.sessions.map((s, i) => (
                        <div
                          ...
                          className="min-w-0 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-700"
                        >
