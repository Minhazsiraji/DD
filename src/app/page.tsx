    ...
    <MarketingShell>
      <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-14 lg:grid-cols-[1.02fr_.98fr] lg:px-8 lg:pb-20 lg:pt-20">
        <div className="self-center">
          <p className="dd-chip inline-flex rounded-full px-3 py-1.5 text-[12px] font-semibold text-[#5f52c5]">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/signup" className="dd-primary inline-flex h-12 items-center gap-2 rounded-full px-5 text-sm font-semibold text-white">
              Start free
              <ArrowRight className="size-4" aria-hidden="true" />
            </Link>
            <Link href="/pricing" className="dd-secondary inline-flex h-12 items-center rounded-full px-5 text-sm font-semibold text-ink-secondary">
              See founding plan
            </Link>
          </div>
        ...
        <div className="dd-landing-demo p-4 sm:p-5">
          <div className="dd-landing-inner rounded-[24px] p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className="dd-chip inline-flex rounded-full px-3 py-1.5 text-[11px] font-semibold text-[#5f52c5]">
                Private clinical workspace
              </span>
            </div>
            <div className="mt-6 grid gap-3">
              {workflow.map((item, i) => (
                <div key={item} className="dd-workflow-row flex items-center gap-3 rounded-[18px] px-4 py-3.5 text-[13.5px] font-medium text-[#48425f]">
                  <span className="dd-step grid size-8 shrink-0 place-items-center rounded-full text-[12px] font-semibold text-[#6554d0]">{i + 1}</span>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-10">
        <div className="grid gap-4 md:grid-cols-3">
          {benefits.map((benefit) => (
            <article key={benefit.title} className="dd-public-card p-5 sm:p-6">
              <span className="dd-feature-icon inline-flex size-10 items-center justify-center rounded-[14px] text-[#6655cf]">
                {benefit.icon}
              </span>
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-16">
        <div className="dd-public-cta px-6 py-8 sm:px-8 lg:flex lg:items-center lg:justify-between lg:px-10">
          <div>
          ...
          <Link href="/pricing" className="dd-secondary mt-6 inline-flex h-11 items-center rounded-full px-5 text-[13px] font-semibold text-ink-secondary lg:mt-0">
            View the plan
          </Link>
        </div>
      </section>
