...
export const DEFAULT_BACKGROUND_COLOR = "#CACACA";
...
async function runImageTransaction<T>(
  ...
  try {
    return await new Promise<T>((resolve, reject) => {
      ...
      transaction.onerror = () => reject(transaction.error ?? new Error("Background storage failed."));
    });
  } finally {
    db.close();
  }
...
export async function deleteBackgroundImage(): Promise<void> {
  await runImageTransaction<void>("readwrite", (store, resolve, reject) => {
    ...
    request.onsuccess = () => resolve();
    request.onerror = () => reject(request.error ?? new Error("Unable to remove background image."));
  });
}
