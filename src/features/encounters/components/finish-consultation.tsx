...
/** Ending the visit. The database action remains the clinical authority. */
export function FinishConsultation({
  ...
  version: number;
  unsaved: boolean;
  /** Any in-flight mutation, save failure, conflict or desync. */
  ...
  const blocked = unsaved || Boolean(blockedReason);
      ...
      <div className="min-w-0 space-y-3 p-4 sm:p-5">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 break-words text-[13px] text-ink-secondary">
            Closing the visit puts it in the patient&rsquo;s history as seen. It does not approve or
            finalise a prescription.
          </p>
        </div>
        ...
        {unsaved ? (
          <p className="flex min-w-0 items-start gap-2 rounded-xl bg-warning-soft px-3 py-2 text-[13px] font-medium text-ink">
            ...
            <span className="min-w-0 break-words">
              Changes are still pending. Wait for autosave, or resolve the save message below.
            </span>
          </p>
        ) : blockedReason ? (
          <p className="flex min-w-0 items-start gap-2 rounded-xl bg-warning-soft px-3 py-2 text-[13px] font-medium text-ink">
            <CircleAlert className="mt-px size-4 shrink-0 text-warning" aria-hidden="true" />
            <span className="min-w-0 break-words">{blockedReason}</span>
          </p>
        ) : null}
        ...
        {confirming ? (
          <div
            ...
            className="dd-material-record dd-record-pearl min-w-0 rounded-xl p-3 sm:p-4"
          >
            ...
            <p className="mt-1 break-words text-[13px] text-ink-secondary">
              The encounter becomes read-only. Prescription approval remains a separate action.
            </p>
