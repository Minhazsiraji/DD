 ...
 *
 * EVERY PATH STILL GOES THROUGH SUPABASE. `exchangeCodeForSession` and
 * `verifyOtp` validate directly; the implicit branch installs the token pair
 * with the Supabase SDK and immediately calls `getUser()` so navigation only
 * follows a server-validated session. What changed is only
...
async function verify(supabase: Client, result: VerifiableLink): Promise<boolean> {
  ...
  /**
   * Implicit flow. Install the token pair through the Supabase SDK, then call
   * `getUser()` below so a forged or otherwise invalid session cannot be
   * accepted merely because it appeared in the fragment.
   */
  ...
  // `setSession` accepts a token pair into the SDK. `getUser()` then asks
  // Supabase Auth to validate the resulting session before navigation.
  const { data, error: userError } = await supabase.auth.getUser();
  return !userError && Boolean(data.user);
}
