...
describe("what a server can see", () => {
  ...
  it("rejects an implicit credential with a missing or unsupported auth type", () => {
    expect(readLinkResult("", "#access_token=a&refresh_token=b")).toEqual({ kind: "none" });
    expect(readLinkResult("", "#access_token=a&refresh_token=b&type=sms")).toEqual({ kind: "none" });
    expect(readLinkResult("", "#access_token=a&refresh_token=b&type=../admin")).toEqual({ kind: "none" });
  });
});
