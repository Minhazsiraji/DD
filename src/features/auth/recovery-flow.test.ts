...
describe("verification is never skipped", () => {
  ...
  it("scrubs before handling a Supabase refusal too", async () => {
    const src = await read("src/features/auth/components/confirm-link.tsx");
    const strips = src.indexOf("history.replaceState");
    const refusal = src.indexOf('result.kind === "error"');
    expect(strips).toBeGreaterThan(-1);
    expect(refusal).toBeGreaterThan(-1);
    expect(strips).toBeLessThan(refusal);
  });

  it("validates an implicit session with Supabase Auth before navigation", async () => {
    const src = await read("src/features/auth/components/confirm-link.tsx");
    const setSession = src.indexOf("setSession(");
    const getUser = src.indexOf("getUser()");
    expect(setSession).toBeGreaterThan(-1);
    expect(getUser).toBeGreaterThan(setSession);
  });
...
describe("request and redirect policy regressions", () => {
  it("signup and recovery both target the scanner-safe confirmation page", async () => {
    const src = await read("src/features/auth/actions.ts");
    expect(src).toMatch(/emailRedirectTo: `\$\{authRedirectOrigin\(\)\}\/auth\/confirm\?next=\/onboarding`/);
    expect(src).toMatch(/redirectTo: `\$\{authRedirectOrigin\(\)\}\/auth\/confirm\?next=\/reset-password`/);
    expect(src).not.toMatch(/auth\/callback\?next=\/(onboarding|reset-password)/);
  });

  it("password recovery remains account-enumeration safe", async () => {
    const src = await read("src/features/auth/actions.ts");
    const start = src.indexOf("export async function forgotPasswordAction");
    const end = src.indexOf("export async function resetPasswordAction", start);
    const recovery = src.slice(start, end);
    expect(recovery).toMatch(/await supabase\.auth\.resetPasswordForEmail/);
    expect(recovery).not.toMatch(/if \(error\)|error\.message|return \{ ok: false/);
    expect(recovery).toMatch(/If an account exists for that address/);
  });

  it("the confirm page suppresses referrer propagation", async () => {
    const src = await read("src/app/auth/confirm/page.tsx");
    expect(src).toMatch(/referrer:\s*"no-referrer"/);
  });
});
