...
export type LinkResult =
  ...
  | { kind: "implicit"; accessToken: string; refreshToken: string; type: EmailOtpType }
...
export function readLinkResult(search: string, hash: string): LinkResult {
  ...
  const implicitType = fragment.get("type");
  if (accessToken && refreshToken && implicitType && OTP_TYPES.has(implicitType)) {
    return {
      kind: "implicit",
      accessToken,
      refreshToken,
      type: implicitType as EmailOtpType,
    };
  }
...
export function safeNextPath(next: string | null): string {
  ...
  try {
    const base = "https://doctors-diary.invalid";
