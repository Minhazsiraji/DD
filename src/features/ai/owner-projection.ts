 ...
 *
 * QUANTITY COMPLETENESS (the same rule, applied to quantities)
 *   If ANY contributing event has an unknown quantity, quantity_total is NULL —
 *   even when its siblings are known. Three events of 100, 200 and UNKNOWN
 *   publish event_count 3 and quantity_total NULL, never 300. Only a count
 *   that is complete by construction (the count rows below, one per event
 *   that exists) may be emitted as a known quantity.
 *
 * COST-BEARING ROWS
...
export function projectOwnerAggregates(
  ...
  const contribute = (
    ...
    if (!bucket) {
      bucket = { key, quantity: 0, anyQuantityUnknown: false, eventCount: 0, costs: [] };
      buckets.set(id, bucket);
    }
    bucket.eventCount += 1;
    // COMPLETENESS, NOT BEST EFFORT: one contributor with an unknown quantity
    // makes the whole bucket quantity unknown. Summing only the known ones
    // would publish a number that looks like a total and is not.
    if (quantity === null) {
      bucket.anyQuantityUnknown = true;
    } else {
  ...
  // THE SERVER-ISSUED GRANT IS THE ONLY ANCHOR. A grant id is a non-secret
  // correlation id, never authority, so a report counts only if it names a
  // grant the server issued to the SAME authenticated user (the report's actor
  // is taken from the server session, never from its body). One grant is one
  // streaming session: the earliest such report wins.
  //
  // Consequences, all deliberate:
  //   - another user holding a leaked grant id can file a report, but it is
  //     attributed to them, pairs with nothing, and cannot move the grant
  //     holder's figures;
  //   - a report naming no issued grant is not counted at all — unverifiable
  //     audio would otherwise let anyone inflate their own usage;
  //   - the worst a hostile report can do is leave a grant unmeasured, which
  //     reads as NULL (unknown), never as a wrong number.
  const grantById = new Map(issued.map((grant) => [grant.grant_id, grant] as const));
  const sessionByGrant = new Map<string, VoiceSessionReportedEvent>();
  for (const session of sessions) {
    const grant = grantById.get(session.grant_id);
    if (!grant || grant.actor_user_id === null || grant.actor_user_id !== session.actor_user_id) {
      continue;
    }
  ...
  return [...buckets.entries()]
    ...
    .map(([, bucket]) => {
      ...
      const row: OwnerAggregateRow = {
        ...
        quantity_total: bucket.anyQuantityUnknown ? null : bucket.quantity,
