...
describe("Partial known sum is never reported as complete — O1-E-R3 mixed example", () => {
  ...
  it("keeps quantity_total as the known-subset sum", () => {
    expect(row(rows, "INPUT_TOKENS").quantity_total).toBe(2400);
    expect(row(rows, "CACHED_INPUT_TOKENS").quantity_total).toBe(200);
    expect(row(rows, "OUTPUT_TOKENS").quantity_total).toBe(114);
  });
...
describe("Voice — an unmeasured session is unknown, never zero", () => {
  ...
  const session = (grantId: string, ms: number) =>
    buildVoiceSessionReported({
      occurredAt: at,
      actorUserId: ACTOR,
      beacon: {
  ...
  it("one grant with no report NULLs the day's audio cost", () => {
    const a = grant();
    const b = grant();
    const rows = project([a.event, session(a.grantId, 2000), b.event]);
    expect(row(rows, "AUDIO_MILLIS")).toMatchObject({ quantity_total: 2000, event_count: 2, estimated_cost_minor: null });
