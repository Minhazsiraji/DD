...
import { MEDICINE_FIELDS, type MedicineDraft, type MedicineField } from "../schema";
import type { SignedMedicineSuggestion } from "../m3-history";

const FAST_KEYS = new Set<keyof MedicineDraft>([
  "displayName",
  "strengthText",
  "doseText",
  "scheduleText",
...
function spanClass(field: MedicineField): string {
  ...
  if (field.key === "scheduleText" || field.key === "durationText") return "col-span-6 sm:col-span-3";
  return field.span === 12 ? "col-span-12" : field.span === 6 ? "col-span-12 sm:col-span-6" : field.span === 4 ? "col-span-6 sm:col-span-4" : "col-span-6 sm:col-span-3";
}
...
export function MedicineForm({
  ...
  const moreHasValue = MORE_FIELDS.some((field) => String(value[field.key] ?? "").trim() !== "") || value.isPrn || !value.substitutionAllowed;

  const renderField = (field: MedicineField) => {
    const isName = field.key === "displayName";
    return (
      <div key={field.key} className={cn(spanClass(field), "relative min-w-0")}>
        <label htmlFor={`${id}-${field.key}`} className="text-[12px] font-semibold text-ink-secondary">{field.label}</label>
        {field.multiline ? (
          <textarea
            id={`${id}-${field.key}`}
            rows={2}
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === "Enter") { event.preventDefault(); if (canSubmit) onSubmit(); }
          ...
          <ul className="dd-material-panel dd-panel-pearl absolute inset-x-0 top-full z-30 mt-1 max-h-64 overflow-y-auto rounded-xl border border-white/70 shadow-soft">
            {visibleSuggestions.map((suggestion, index) => (
              <li key={`${suggestion.displayName}-${suggestion.strengthText}-${index}`}>
                ...
                >
                  <span className="text-[13px] font-semibold text-ink">{suggestion.displayName}{suggestion.strengthText ? ` ${suggestion.strengthText}` : ""}</span>
                  <span className="text-[11px] text-ink-muted">{[suggestion.doseText, suggestion.scheduleText, suggestion.durationText].filter(Boolean).join(" · ") || "Signed wording"}</span>
                </button>
              </li>
            ))}
            <li className="border-t border-white/60 px-3 py-2 text-[10px] text-ink-muted">Signed prescription history. Selection fills this form; it does not add a medicine.</li>
          </ul>
        ) : null}
            ...
            {field.options.map((option) => (
              <button
                ...
                className={cn(
                  ...
                  value[field.key] === option ? "border-brand bg-brand-soft font-semibold text-brand" : "border-hairline bg-white/70 text-ink-secondary hover:bg-white",
                )}
              >{option}</button>
            ))}
          </div>
  ...
  return (
    ...
    >
      <div>
        <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-brand">Fast entry</p>
        <div className="grid grid-cols-12 gap-3">{FAST_FIELDS.map(renderField)}</div>
      </div>

      <details className="dd-material-record dd-record-pearl rounded-2xl" open={moreHasValue || undefined}>
        <summary className="flex min-h-11 cursor-pointer items-center gap-2 px-3 text-[12px] font-semibold text-ink focus-visible:focus-ring">
          ...
          <span className="ml-auto text-[10px] font-normal text-ink-muted">brand · generic · form · route · quantity · food · instructions</span>
        </summary>
        <div className="border-t border-white/55 p-3">
          ...
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
            <label className="flex min-h-11 cursor-pointer items-center gap-2 text-[12px] text-ink">
              <input type="checkbox" checked={value.isPrn} disabled={busy} onChange={(event) => set("isPrn", event.target.checked)} className="size-4 accent-[var(--color-brand)]" />As needed (PRN)
            </label>
            <label className="flex min-h-11 cursor-pointer items-center gap-2 text-[12px] text-ink">
              <input type="checkbox" checked={value.substitutionAllowed} disabled={busy} onChange={(event) => set("substitutionAllowed", event.target.checked)} className="size-4 accent-[var(--color-brand)]" />Substitution allowed
            </label>
          </div>
        </div>
      </details>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        <button type="submit" disabled={!canSubmit} className="dd-primary inline-flex min-h-11 items-center justify-center gap-1.5 px-4 text-[13px] font-semibold disabled:cursor-not-allowed disabled:opacity-55 focus-visible:focus-ring">
          {busy ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <Check className="size-4" aria-hidden="true" />}
          {busy ? "Saving…" : submitLabel}
        </button>
        <button type="button" onClick={onCancel} disabled={busy} className="dd-secondary inline-flex min-h-11 items-center justify-center px-4 text-[13px] font-semibold disabled:opacity-55 focus-visible:focus-ring">Cancel</button>
        <p className="basis-full text-[10px] text-ink-muted">Medicine name is the only required field. Press Enter from a single-line field to add quickly.</p>
      </div>
    </form>
  );
}
