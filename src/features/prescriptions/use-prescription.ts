...
export function usePrescription(
   ...
   *
   * M3 historical reuse deliberately joins this SAME coordinator instead of
   * inventing a second mutation channel: one MutationGate, one prescription
   * version, one recovery table.
   *
  ...
  const run = React.useCallback(
    async (
      ...
      if (result.ok) {
        ...
        // A move/reuse has no editor to close; everything else submitted a form.
  ...
  /**
   * M3 history selection is a proposal, never a write. Opening the add editor is
   * the entire effect; the Doctor may inspect/edit every field before an
   * explicit Add medicine reaches `add_prescription_item`.
   */
  const proposeMedicine = React.useCallback(
    (suggestion: MedicineDraft) => {
  ...
  /**
   * Whole/selected historical reuse is the only M3 history action that writes.
   * It uses the accepted atomic RPC and still passes through `run`, so CAS,
   * MutationGate, unknown-outcome handling and a fresh canonical re-read remain
   * identical to ordinary medicine edits.
   */
  const reuseHistory = React.useCallback(
    async (input: {
      ...
      // Reuse and an open editor cannot coexist. The Doctor must finish/cancel
      // the proposed medicine first, so no unsaved text is hidden by a bulk write.
      if (liveEditor.current) return null;
      return run(
        (expectedVersion) =>
          reusePrescriptionItemsAction({
            prescriptionId,
            expectedVersion,
            ...input,
          }),
