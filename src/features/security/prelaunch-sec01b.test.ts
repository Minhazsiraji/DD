...
describe("PRE-LAUNCH-SEC-01B-R1 forward security candidate", () => {
  ...
  it("does not log known patient/encounter identifiers or raw errors in scrubbed surfaces", () => {
    ...
    expect(sources).not.toMatch(/console\.error\([^;]*(?:patientId|encounterId)/);
    expect(sources).not.toMatch(/console\.error\([^;]*(?:error|Error)\.message/);
    expect(sources).not.toMatch(/console\.error\([^;]*(?:previous|path)(?:\s*,|\))/);
  });
});
