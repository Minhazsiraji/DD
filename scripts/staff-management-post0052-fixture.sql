...
create type public.visit_type as enum ('NEW','FOLLOW_UP','REPORT_REVIEW','PROCEDURE','EMERGENCY');create type public.encounter_status as enum ('DRAFT','COMPLETED','CANCELLED');
...
create table public.practice_location_members (
  ...
  status public.member_status not null default 'ACTIVE',  invited_by uuid references public.profiles(id),
...
create table public.patient_location_links (
  ...
  practice_location_id uuid not null references public.practice_locations(id),  first_seen_at timestamptz not null default now(),
...
create table public.queue_events (
  id uuid primary key default gen_random_uuid(),
  appointment_id uuid not null references public.appointments(id),  practice_location_id uuid not null references public.practice_locations(id),
...
create table public.patient_documents (
  id uuid primary key default gen_random_uuid(),
  patient_id uuid not null references public.patients(id),  owner_doctor_id uuid not null references public.doctor_profiles(id),
  practice_location_id uuid not null references public.practice_locations(id),
...
grant select on public.doctor_profiles, public.practice_locations, public.practice_location_members,
  ...
  public.queue_entries, public.audit_events to authenticated;alter table public.doctor_chambers enable row level security;
...
-- Minimal pre-0053 Doctor ownership policies, matching the current owner boundary.
...
revoke all on public.patient_documents, public.encounters, public.appointment_events,
  public.queue_events, public.appointment_token_counters from authenticated, anon;
revoke all on all tables in schema public from anon;
-- Clinical authority sentinels: current protected names and write posture.
...
revoke all on public.prescriptions from authenticated, anon;
