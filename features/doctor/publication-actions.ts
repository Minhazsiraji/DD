...
/**
 * Publish/unpublish the signed-in doctor's own professional profile.
 *
 * Intentionally accepts no doctor/profile/user id. The target row is derived
 * from auth.uid() and doctor_profiles RLS independently enforces the same
 * ownership boundary. Reception/admin accounts therefore cannot acquire doctor
 * publication authority merely by reaching this server action.
 */
export async function setProfileVisibilityAction(input: unknown): Promise<PublicationResult> {
  ...
  if (!slug) {
    return { ok: false, message: "Choose and save your public profile link before publishing." };
  }
