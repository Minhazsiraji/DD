...
| D7 | **CRLF working tree breaks 4 tests on Windows** — see **CAE-FINDING-006** (§8.3). `core.autocrlf=true` + no `.gitattributes` rule for `src/**` / `supabase/**` means the working tree is CRLF; 4 LF-exact assertions fail locally while passing in CI. | MD2 / CENTRAL |
...
> Fixing it is a tracked-file change outside CAE-02's authorised surface.
> **Recommended owner: MD2 / CENTRAL.** Options: (a) add
> `src/** text=auto eol=lf` and `supabase/** text=auto eol=lf` to
> `.gitattributes` and renormalise on a quiet window; or (b) make those four
> assertions line-ending-insensitive — precedent exists on
> `feature/doctor-profile-claim` ("Stop the signature test depending on line
> endings"). Until then, on Windows run `git config core.autocrlf false` and
> re-checkout, or run the suite in WSL/CI, to see gate 2 green.

`repo-health --strict` also confirms the seven frozen artifacts still match at
`130f1ab` (frozen integrity intact) and `origin/main` is unchanged.
