...
Deno.serve(async (req) => {
  ...
  if (!projectUrl || !serviceRoleKey) {
    return Response.json({ photoUrl: null }, { status: 500, headers: noStoreHeaders() });
  }
  ...
  // The caller never chooses this path. A malformed/stale database value fails closed.
  ...
  const photoUrl = safeProjectHttpsUrl(signed?.signedUrl ?? null, projectUrl);
...
function safeProjectHttpsUrl(value: string | null, projectUrl: string): string | null {
