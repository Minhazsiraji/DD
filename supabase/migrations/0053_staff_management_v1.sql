...
-- AUTHORING CHECKPOINT ONLY: DO NOT APPLY TO PROTECTED SUPABASE WITHOUT CENTRAL APPROVAL.
--
...
-- RECEPTIONIST membership there and uses the inert ASSISTANT location role only
-- as the broad location-entry marker. The semantic Team role remains on the
-- Doctor-specific grant. This is required for shared-clinic Doctor A/B isolation.
...
create table public.doctor_staff_invitations (
  ...
  constraint doctor_staff_invitation_email_normalized
    check (email = lower(btrim(email)) and length(email) between 3 and 320),
  constraint doctor_staff_invitation_locations_nonempty
    check (cardinality(requested_location_ids) > 0),
  constraint doctor_staff_invitation_expiry_valid check (expires_at > created_at),
  constraint doctor_staff_invitation_terminal_state
    check (not (linked_at is not null and revoked_at is not null))
);

create unique index doctor_staff_invitation_pending_email_key
  on public.doctor_staff_invitations (doctor_profile_id, email)
  where linked_at is null and revoked_at is null;
create index doctor_staff_invitation_doctor_idx
...
-- Assistant investigation work is a proposal, never a clinical order.
...
-- ASSISTANT is intentionally an inert managed-entry marker in application RBAC.
-- Text comparison avoids unsafe direct use of the newly-added enum value inside
...
-- Service-role-only invitation finalization. Invitation alone confers nothing.
...
begin
  ...
  select * into inv from public.doctor_staff_invitations
  where id = target_invitation_id for update;
  if not found or inv.linked_at is not null or inv.revoked_at is not null or inv.expires_at <= now() then
  ...
  select d.user_id into doctor_user_id from public.doctor_profiles d
  where d.id = inv.doctor_profile_id;
  ...
  if cardinality(inv.requested_location_ids) = 0
     or cardinality(inv.requested_location_ids) <>
        (select count(distinct x)::int from unnest(inv.requested_location_ids) x) then
  ...
  if cardinality(inv.requested_location_ids) <>
     (select count(*)::int from public.doctor_chambers c
      where c.doctor_profile_id = inv.doctor_profile_id
        and c.practice_location_id = any(inv.requested_location_ids)) then
  ...
  select g.id into grant_id from public.doctor_staff_grants g
  ...
  if found then
    if exists (
      ...
      where g.id = grant_id and g.status <> 'REMOVED'
    ) then
      raise exception 'STAFF_RELATIONSHIP_ALREADY_EXISTS' using errcode = '23505';
    end if;
    update public.doctor_staff_grants
    set role = inv.role, status = 'ACTIVE', source_invitation_id = inv.id,
        starts_at = now(), ends_at = null, temporarily_disabled_at = null,
        removed_at = null, updated_by = inv.created_by, updated_at = now()
    where id = grant_id;
    delete from public.doctor_staff_locations where grant_id = grant_id;
    delete from public.doctor_staff_permissions where grant_id = grant_id;
  else
    insert into public.doctor_staff_grants (
      doctor_profile_id, staff_user_id, role, source_invitation_id, created_by, updated_by
    ) values (
      inv.doctor_profile_id, target_staff_user_id, inv.role, inv.id, inv.created_by, inv.created_by
    ) returning id into grant_id;
  end if;

  foreach loc in array inv.requested_location_ids loop
    -- Explicit adoption ends the legacy location-wide receptionist authority at
    -- this location. This does NOT touch any unadopted account/location.
    ...
    select m.status::text into managed_status
    ...
    if managed_status = 'SUSPENDED' then
      ...
      using loc, target_staff_user_id, inv.created_by;
    end if;

    insert into public.doctor_staff_locations (grant_id, doctor_profile_id, practice_location_id)
    values (grant_id, inv.doctor_profile_id, loc);
  end loop;

  foreach perm in array inv.requested_permissions loop
    insert into public.doctor_staff_permissions (grant_id, permission) values (grant_id, perm);
  end loop;
  ...
  insert into public.audit_events (
    ...
    inv.requested_location_ids[1], inv.created_by, 'STAFF_LINKED',
    'doctor_staff_grant', grant_id,
    jsonb_build_object(
...
set search_path = public, pg_temp
as $$
declare g public.doctor_staff_grants%rowtype;
begin
  ...
  if g.status = 'REMOVED' and target_status <> 'REMOVED' then
...
set search_path = public, pg_temp
as $$
declare g public.doctor_staff_grants%rowtype; perm public.doctor_staff_permission;
begin
  perform public.require_aal2();
  select * into g from public.doctor_staff_grants where id = target_grant_id for update;
  ...
  if exists (
    select 1 from unnest(coalesce(target_permissions,'{}'::public.doctor_staff_permission[])) p
    where not public.staff_role_allows_permission(g.role,p)
  ) then
    raise exception 'STAFF_PERMISSION_EXCEEDS_ROLE' using errcode = '42501';
  end if;
  ...
  foreach perm in array coalesce(target_permissions,'{}'::public.doctor_staff_permission[]) loop
    insert into public.doctor_staff_permissions (grant_id,permission) values (g.id,perm) on conflict do nothing;
  end loop;
...
set search_path = public, pg_temp
as $$
declare g public.doctor_staff_grants%rowtype; loc uuid; entry_status text;
begin
  perform public.require_aal2();
  select * into g from public.doctor_staff_grants where id=target_grant_id for update;
  if not found or g.doctor_profile_id<>public.current_doctor_id() or g.status='REMOVED' then
  ...
  if target_location_ids is null or cardinality(target_location_ids)=0
     or cardinality(target_location_ids)<>(select count(distinct x)::int from unnest(target_location_ids)x) then
    raise exception 'STAFF_LOCATION_SET_INVALID' using errcode='22023';
  end if;
  if cardinality(target_location_ids)<>(
    select count(*)::int from public.doctor_chambers c
    where c.doctor_profile_id=g.doctor_profile_id and c.practice_location_id=any(target_location_ids)
  ) then raise exception 'STAFF_LOCATION_NOT_DOCTOR_CHAMBER' using errcode='42501'; end if;

  delete from public.doctor_staff_locations where grant_id=g.id;
  foreach loc in array target_location_ids loop
    update public.practice_location_members m set status='SUSPENDED',updated_at=now()
    where m.practice_location_id=loc and m.user_id=g.staff_user_id
      and m.role::text='RECEPTIONIST' and m.status='ACTIVE';

    select m.status::text into entry_status from public.practice_location_members m
    where m.practice_location_id=loc and m.user_id=g.staff_user_id and m.role::text='ASSISTANT' limit 1;
    if entry_status='SUSPENDED' then raise exception 'STAFF_MANAGED_ENTRY_SUSPENDED' using errcode='42501';
    elsif entry_status='INVITED' then
      update public.practice_location_members m set status='ACTIVE',joined_at=coalesce(joined_at,now()),updated_at=now()
      where m.practice_location_id=loc and m.user_id=g.staff_user_id and m.role::text='ASSISTANT';
    elsif entry_status is null then
      execute 'insert into public.practice_location_members '
        ||'(practice_location_id,user_id,role,status,invited_by,joined_at) '
        ||'values ($1,$2,''ASSISTANT''::public.location_role,''ACTIVE'',$3,now())'
      using loc,g.staff_user_id,auth.uid();
    end if;
    insert into public.doctor_staff_locations(grant_id,doctor_profile_id,practice_location_id)
...
-- Narrow clinical-support RPCs. No prescription function is added.
...
set search_path=public,pg_temp
as $$
declare e public.encounters%rowtype; grant_id uuid; staff_role public.doctor_staff_role;
begin
  perform public.require_aal2();
  select * into e from public.encounters where id=target_encounter_id for update;
  if not found or e.status<>'DRAFT' then raise exception 'STAFF_INTAKE_ENCOUNTER_NOT_DRAFT' using errcode='42501'; end if;
  ...
  select g.id,g.role into grant_id,staff_role from public.doctor_staff_grants g
  join public.doctor_staff_locations l on l.grant_id=g.id and l.practice_location_id=e.practice_location_id
  where g.doctor_profile_id=e.owner_doctor_id and g.staff_user_id=auth.uid() and g.status='ACTIVE' limit 1;
  update public.encounters set
    vital_height_cm=target_height_cm,vital_weight_kg=target_weight_kg,
    vital_temperature_c=target_temperature_c,vital_pulse_bpm=target_pulse_bpm,
    vital_systolic=target_systolic,vital_diastolic=target_diastolic,
    vital_resp_rate=target_resp_rate,vital_spo2=target_spo2,updated_at=now()
  where id=e.id;
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(e.practice_location_id,auth.uid(),'STAFF_INTAKE_VITALS_UPDATED','encounter',e.id,
         jsonb_build_object('doctor_profile_id',e.owner_doctor_id,'staff_role',staff_role,'grant_id',grant_id));
end;
$$;

create or replace function public.staff_prepare_investigation(
  target_encounter_id uuid,target_name text,target_note text default null
)
returns uuid
language plpgsql
security definer
set search_path=public,pg_temp
as $$
declare e public.encounters%rowtype; grant_id uuid; staff_role public.doctor_staff_role; proposal_id uuid;
begin
  perform public.require_aal2();
  select * into e from public.encounters where id=target_encounter_id;
  if not found or e.status<>'DRAFT' then raise exception 'STAFF_INVESTIGATION_ENCOUNTER_NOT_DRAFT' using errcode='42501'; end if;
  ...
  ) values(grant_id,e.owner_doctor_id,e.practice_location_id,e.id,auth.uid(),btrim(target_name),target_note)
  returning id into proposal_id;
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(e.practice_location_id,auth.uid(),'STAFF_INVESTIGATION_PROPOSED','staff_investigation_proposal',proposal_id,
         jsonb_build_object('doctor_profile_id',e.owner_doctor_id,'staff_role',staff_role,'grant_id',grant_id,'encounter_id',e.id));
  return proposal_id;
end;
$$;

create or replace function public.staff_attach_document_record(
  target_doctor_id uuid,target_location_id uuid,target_patient_id uuid,target_encounter_id uuid,
  target_document_type public.document_type,target_title text,target_document_date date,
  target_notes text,target_storage_path text,target_mime_type text,target_size_bytes integer,
...
set search_path=public,pg_temp
as $$
declare document_id uuid; staff_role public.doctor_staff_role; grant_id uuid;
begin
  perform public.require_aal2();
  if not public.has_doctor_staff_permission(target_doctor_id,target_location_id,'document.attach') then
    raise exception 'STAFF_DOCUMENT_ATTACH_FORBIDDEN' using errcode='42501';
  end if;
  if not exists(select 1 from public.patients p where p.id=target_patient_id and p.owner_doctor_id=target_doctor_id)
     or not exists(select 1 from public.patient_location_links l where l.patient_id=target_patient_id and l.practice_location_id=target_location_id) then
    raise exception 'STAFF_DOCUMENT_PATIENT_NOT_VISIBLE' using errcode='42501';
  end if;
  if target_encounter_id is not null and not exists(
    select 1 from public.encounters e where e.id=target_encounter_id
      and e.owner_doctor_id=target_doctor_id and e.patient_id=target_patient_id
      and e.practice_location_id=target_location_id
  ) then raise exception 'STAFF_DOCUMENT_ENCOUNTER_INVALID' using errcode='42501'; end if;
  select g.id,g.role into grant_id,staff_role from public.doctor_staff_grants g
  join public.doctor_staff_locations l on l.grant_id=g.id and l.practice_location_id=target_location_id
  where g.doctor_profile_id=target_doctor_id and g.staff_user_id=auth.uid() and g.status='ACTIVE' limit 1;
  insert into public.patient_documents(
    patient_id,owner_doctor_id,practice_location_id,encounter_id,document_type,title,
    document_date,notes,storage_path,mime_type,size_bytes,original_filename,uploaded_by
  ) values(
    target_patient_id,target_doctor_id,target_location_id,target_encounter_id,target_document_type,btrim(target_title),
    target_document_date,target_notes,target_storage_path,target_mime_type,target_size_bytes,target_original_filename,auth.uid()
  ) returning id into document_id;
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(target_location_id,auth.uid(),'STAFF_DOCUMENT_ATTACHED','patient_document',document_id,
         jsonb_build_object('doctor_profile_id',target_doctor_id,'staff_role',staff_role,'grant_id',grant_id,'patient_id',target_patient_id));
  return document_id;
end;
$$;
...
-- Doctor-scoped operational RPCs for Team-managed receptionists. Existing
-- location-wide pilot receptionist RPCs remain unchanged for unadopted staff.
create or replace function public.staff_create_appointment(
  target_doctor_id uuid,target_location_id uuid,target_patient_id uuid,
  target_scheduled_for timestamptz,target_duration_minutes integer default 15,
  target_visit_type public.visit_type default 'NEW',target_reason text default null
)
returns uuid
language plpgsql
security definer
set search_path=public,pg_temp
as $$
declare appointment_id uuid; session_day date;
begin
  perform public.require_aal2();
  if not public.has_doctor_staff_permission(target_doctor_id,target_location_id,'appointments.manage') then
    raise exception 'STAFF_APPOINTMENT_MANAGE_FORBIDDEN' using errcode='42501'; end if;
  if not public.doctor_practises_at(target_doctor_id,target_location_id) then
    raise exception 'STAFF_APPOINTMENT_DOCTOR_LOCATION_INVALID' using errcode='42501'; end if;
  if not exists(select 1 from public.patients p where p.id=target_patient_id and p.owner_doctor_id=target_doctor_id)
     or not exists(select 1 from public.patient_location_links l where l.patient_id=target_patient_id and l.practice_location_id=target_location_id) then
    raise exception 'STAFF_APPOINTMENT_PATIENT_INVALID' using errcode='42501'; end if;
  if target_scheduled_for is null then raise exception 'STAFF_APPOINTMENT_TIME_REQUIRED' using errcode='22023'; end if;
  session_day:=public.session_date_for(target_location_id,target_scheduled_for);
  insert into public.appointments(
    owner_doctor_id,practice_location_id,patient_id,scheduled_for,session_date,
    duration_minutes,visit_type,reason,created_by
  ) values(target_doctor_id,target_location_id,target_patient_id,target_scheduled_for,session_day,
           coalesce(target_duration_minutes,15),coalesce(target_visit_type,'NEW'),
           nullif(btrim(coalesce(target_reason,'')),''),auth.uid())
  returning id into appointment_id;
  insert into public.appointment_events(appointment_id,practice_location_id,event_type,to_status,actor_id)
  values(appointment_id,target_location_id,'CREATED','SCHEDULED',auth.uid());
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(target_location_id,auth.uid(),'STAFF_APPOINTMENT_CREATED','appointment',appointment_id,
...
create or replace function public.staff_set_appointment_status(
  target_appointment_id uuid,target_status public.appointment_status,
  target_reason public.cancellation_reason default null,target_note text default null
)
...
set search_path=public,pg_temp
as $$
declare a public.appointments%rowtype; event_type public.appointment_event_type; new_token integer; required_perm public.doctor_staff_permission;
begin
  perform public.require_aal2();
  select * into a from public.appointments where id=target_appointment_id for update;
  if not found then raise exception 'STAFF_APPOINTMENT_NOT_FOUND' using errcode='42501'; end if;
  if target_status in ('IN_CONSULTATION','COMPLETED') then
    raise exception 'STAFF_CLINICAL_APPOINTMENT_TRANSITION_FORBIDDEN' using errcode='42501'; end if;
  required_perm:=case when target_status='ARRIVED' then 'arrival.manage'::public.doctor_staff_permission
                      else 'appointments.manage'::public.doctor_staff_permission end;
  if not public.has_doctor_staff_permission(a.owner_doctor_id,a.practice_location_id,required_perm) then
    raise exception 'STAFF_APPOINTMENT_STATUS_FORBIDDEN' using errcode='42501'; end if;
  if a.status=target_status then return a.status; end if;
  if not public.appointment_transition_allowed(a.status,target_status) then
    raise exception 'STAFF_APPOINTMENT_TRANSITION_INVALID' using errcode='22023'; end if;
  if target_status='CANCELLED' and target_reason is null then
    raise exception 'STAFF_APPOINTMENT_CANCELLATION_REASON_REQUIRED' using errcode='22023'; end if;
  event_type:=case target_status
    when 'CONFIRMED' then 'CONFIRMED' when 'ARRIVED' then 'ARRIVED'
    when 'CANCELLED' then 'CANCELLED' when 'NO_SHOW' then 'NO_SHOW'
    else null end;
  if event_type is null then raise exception 'STAFF_APPOINTMENT_TRANSITION_FORBIDDEN' using errcode='42501'; end if;
  if target_status='ARRIVED' and a.token_number is null then
    new_token:=public.allocate_token(a.practice_location_id,a.session_date); end if;
  update public.appointments set
    status=target_status,token_number=coalesce(new_token,token_number),
    ...
    cancellation_note=coalesce(nullif(btrim(coalesce(target_note,'')),''),cancellation_note),updated_at=now()
  where id=a.id;
  insert into public.appointment_events(appointment_id,practice_location_id,event_type,from_status,to_status,actor_id,note)
  values(a.id,a.practice_location_id,event_type,a.status,target_status,auth.uid(),nullif(btrim(coalesce(target_note,'')),''));
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(a.practice_location_id,auth.uid(),'STAFF_APPOINTMENT_STATUS_CHANGED','appointment',a.id,
         jsonb_build_object('doctor_profile_id',a.owner_doctor_id,'from_status',a.status,'to_status',target_status));
  return target_status;
end;
$$;

create or replace function public.staff_set_queue_priority(
  target_appointment_id uuid,target_location_id uuid,target_reason public.priority_reason,target_note text default null
)
returns void
language plpgsql
security definer
...
begin
  perform public.require_aal2();
  select * into a from public.appointments where id=target_appointment_id for update;
  if not found or a.practice_location_id<>target_location_id or a.status<>'ARRIVED'
     or not public.has_doctor_staff_permission(a.owner_doctor_id,target_location_id,'queue.manage') then
    raise exception 'STAFF_QUEUE_FORBIDDEN' using errcode='42501'; end if;
  if target_reason is null then raise exception 'STAFF_QUEUE_REASON_REQUIRED' using errcode='22023'; end if;
  insert into public.queue_entries(appointment_id,practice_location_id,session_date)
  values(a.id,a.practice_location_id,a.session_date)
  on conflict(appointment_id) do update set updated_at=now();
  update public.queue_entries set priority=1,priority_reason=target_reason,
    priority_note=nullif(btrim(coalesce(target_note,'')),''),priority_set_by=auth.uid(),updated_at=now()
  where appointment_id=a.id;
  insert into public.queue_events(appointment_id,practice_location_id,event_type,reason,note,actor_id)
  values(a.id,a.practice_location_id,'PRIORITY_SET',target_reason,nullif(btrim(coalesce(target_note,'')),''),auth.uid());
  insert into public.audit_events(practice_location_id,actor_id,action,resource_type,resource_id,meta)
  values(a.practice_location_id,auth.uid(),'STAFF_QUEUE_PRIORITY_SET','appointment',a.id,
         jsonb_build_object('doctor_profile_id',a.owner_doctor_id));
end;
$$;
...
create policy doctor_staff_invitations_select_doctor on public.doctor_staff_invitations
for select to authenticated using(doctor_profile_id=public.current_doctor_id());
create policy doctor_staff_invitations_insert_doctor on public.doctor_staff_invitations
for insert to authenticated with check(
  doctor_profile_id=public.current_doctor_id() and created_by=auth.uid() and public.session_is_aal2()
);
create policy doctor_staff_grants_select_doctor on public.doctor_staff_grants
for select to authenticated using(doctor_profile_id=public.current_doctor_id());
create policy doctor_staff_locations_select_doctor on public.doctor_staff_locations
for select to authenticated using(doctor_profile_id=public.current_doctor_id());
create policy doctor_staff_permissions_select_doctor on public.doctor_staff_permissions
for select to authenticated using(exists(
  select 1 from public.doctor_staff_grants g
  where g.id=doctor_staff_permissions.grant_id and g.doctor_profile_id=public.current_doctor_id()
));
create policy staff_investigation_proposals_select_doctor_or_actor on public.staff_investigation_proposals
for select to authenticated using(owner_doctor_id=public.current_doctor_id() or proposed_by=auth.uid());

-- Additive delegated read policies. Existing Doctor-owned clinical policies and
-- unadopted legacy receptionist policies are not replaced.
create policy doctor_chambers_select_team_staff on public.doctor_chambers
for select to authenticated using(public.has_doctor_staff_permission(doctor_profile_id,practice_location_id,'chamber.view'));
create policy patients_select_team_staff on public.patients
for select to authenticated using(public.has_doctor_staff_permission_for_patient(owner_doctor_id,id,'patient.lookup'));
create policy appointments_select_team_staff on public.appointments
...
create policy queue_entries_select_team_staff on public.queue_entries
for select to authenticated using(exists(
  select 1 from public.appointments a where a.id=queue_entries.appointment_id
    and public.has_doctor_staff_permission(a.owner_doctor_id,a.practice_location_id,'queue.manage')
));

-- Doctor reads only their Doctor-context Team events; no new Location Admin path.
create policy audit_events_select_doctor_team on public.audit_events
for select to authenticated using(
